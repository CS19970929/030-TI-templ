...
void Init_Charger_AllSeries(void)
{
	...
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR, ENABLE); // 使能PWR外设时钟，待机模式，RTC，看门狗
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0; // é??????????¨???GPIO??????
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;
	...
	EXTI_Init(&EXTI_InitStruct);
	// ??????????????????
	NVIC_InitStructure.NVIC_IRQChannel = EXTI0_1_IRQn; // ?????????é??WK_UP?????¨????¤?é?¨??????é??é??
	NVIC_InitStructure.NVIC_IRQChannelPriority = 0x00; // ???????????????0
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;	   // ???????¤?é?¨??????é??é??
	NVIC_Init(&NVIC_InitStructure);
