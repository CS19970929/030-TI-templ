...
void UartClient_AckDataDeal_0x01(struct RS485MSG *s, UINT8 t_u8BuffTemp[])
{
	...
	if (s->u16RdRegStartAddrActure >= 0x0081)
	{ // 并机时第二组电池单体电压信息
		u16SciTemp = 0;
		for (j = 0; j < 16; j++)
		{
	...
	else if (s->u16RdRegStartAddrActure >= 0x0031)
	{ // 并联时上报第2组电池Spec and Status Query信息
		u16SciTemp = 0;
		for (j = 0; j < 64; j++)
		{
			t_u8BuffTemp[i++] = (u16SciTemp >> 8) & 0x00FF;
...
void UartClient_DataDeal_0x01(struct RS485MSG *s)
{
	...
	if (t_u16Temp >= 0x0081 && t_u16Temp <= 0x0090)
	{ // Box并联时上报第2组电池单体电压信息
		t_u16Temp -= 0x0081;
	}
	else if (t_u16Temp >= 0x0071 && t_u16Temp <= 0x0080)
	{ // 16个电池串电压
		t_u16Temp -= 0x0071;
	}
	else if (t_u16Temp == 0x0070)
	{ // 用于识别同一个BMS下的不同电池组的信息
		t_u16Temp -= 0x0070;
	}
	else if (t_u16Temp >= 0x0031 && t_u16Temp <= 0x006F)
	{ // Box并联时上报第2组电池Spec and Status Query信息
		t_u16Temp = 0x0031;
	}
	else if (t_u16Temp >= 0x0010 && t_u16Temp <= 0x0030)
	{ // 状态查询
		t_u16Temp -= 0x0010;
	}
	else if (t_u16Temp >= 0x0001 && t_u16Temp <= 0x000F)
	{ // 15个说明查询
		t_u16Temp -= 0x0001;
	}
	else
...
void UartClient_Tx_Deal_SCI1(struct RS485MSG *s)
{
	...
	if (gu16_UartClientCommuErrCnt_SCI1)
	{ // 出现奇偶校验错误也得把数据全部接收完，然后不回复
		s->ptr_no = 0;
		s->csr = RS485_STA_TX_COMPLETE;
...
void UartClient_Tx_Deal_SCI2(struct RS485MSG *s)
{
	...
	if (gu16_UartClientCommuErrCnt_SCI2)
	{ // 出现奇偶校验错误也得把数据全部接收完，然后不回复
		s->ptr_no = 0;
		s->csr = RS485_STA_TX_COMPLETE;
